...
export default function MultiUserStats() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        if (data.length === 0) {
          showSuccess('查詢成功，但沒有找到數據');
        } else {
          showSuccess(`查詢成功，找到 ${data.length} 個用戶的數據`);
        }
      } else {
        showError(message);
      }
      ...
      console.error(error);
      showError('查詢失敗: ' + (error.response?.data?.message || error.message));
    }
    setSearching(false);
  };
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      console.error(error);
      showError('導出失敗: ' + (error.response?.data?.message || error.message));
    }
    setSearching(false);
  };
  ...
  const quotaChartData = {
    options: {
      ...
      yaxis: {
        title: { text: '額度消耗', style: { color: theme.palette.text.primary } },
        labels: {
    ...
    <>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
        <Stack direction="column" spacing={1}>
          <Typography variant="h2">多用戶Token統計</Typography>
          <Typography variant="subtitle1" color="text.secondary">
            Multi-User Token Statistics
          </Typography>
          <Typography variant="body2" color="text.secondary">
            查詢多個用戶的所有Token使用情況統計數據
          </Typography>
        </Stack>
      </Stack>
      ...
      <Card sx={{ mb: 3, p: 3 }}>
        <Stack spacing={3}>
          <TextField
            fullWidth
            label="用戶名 (多個用戶名用逗號分隔)"
            placeholder="例如: user1, user2, user3"
            value={usernames}
            onChange={(e) => setUsernames(e.target.value)}
            helperText="輸入一個或多個用戶名，用逗號分隔"
          />
